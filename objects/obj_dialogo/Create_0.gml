...
cria_dialogo = function(_dialogo)
{
	...
	var _ret_altura = (sprite_get_height(_retrato) *_retrato_escala)/2
	var _ret_y = _margem +  _ret_altura;
	var _ret_larg = _retrato_escala * sprite_get_width(_retrato);
	var _qtd_pag = array_length(_dialogo.texto)-1;
	var _margem_caixa = 64;
	
	var _fala = _dialogo.som[pag];
	
	var _acao = global.input[global.tipo_controle][CONTROLE.JUMP_S];
	...
	if (escala_caixa >= .9)
	{
		...
		if (_acao)
		{
			...
			{
				//Mutando o som
				for (var _i = 0; _i < pag; _i++)
				{
					audio_stop_sound(_dialogo.som[_i])	
				}
				libera_player();
				audio_stop_sound(_fala)	
			}
		...
		if (!audio_is_playing(_fala)) and (indice < _txt_tam) and (indice > 2) 
		{
			audio_play_sound(_fala,1,0);	
			audio_sound_gain(_fala,global.volume_dialogos/10,0);
		}
